...
const $room_max = 500			; max number of rooms, (0 = no limit)
const $room_fails_max = 1500		; max number of failed attempts to place a room before we give up
global $room_count = 0				; num of successfully placed rooms
global $room_fails_count = 0		; num of failed attempts to place a room
global $connection_distance_max = 10 	; maximum connection distance, shorter = shorter hallways and denser
...
func make_room()
   ...
   if $flag_overlap then	; room failed to be placed
	  ...
	  if $room_count > 1 then									;ok
		 ...
		 if $goal <> false then	; success
			draw_hallway($x_center, $y_center, $goal[1], $goal[0])	; y x
		 else
			fill("?", "░")		; try again, not a fail
			$room_fails_count -= 1
		 endif
...
func find_closest_room($x_this, $y_this)
   ...
   for $checking = 0 to ubound($xx) - 1 step 1
   ...
   if $distance_best > $connection_distance_max then	;rooms are too far apart, redo
	  return false
   else								; rooms are close enough
	  $result[0] = $yy[$closest]
	  $result[1] = $xx[$closest]
	  return $result
   endif
